...
=head1 Usage
    ...
    -kcpu <int>		    set the cpu number to use in parallel, default=3
...
my ($piler,$piler_cuts,$piler_resource,$repeatscout,$ltr_finder,$tRNA, $rank);
...
my $change_ID = parse_config($config_file,"change_ID"); 
...
open(OUT , ">" . "STEP04_repeat_statistics.sh") or die $!;
    ...
    $stat_parameter = $stat_parameter."-rank $rank" if (defined $rank);
