...
=head1 Usage
	...
	-Interpro		run Interpro
	  ...
	  -iresource <str>	set the required resource used in qsub -l option, default vf=16G
	-KEGG			run KEGG
	...
	-NR				run NR
	-GO				run GO
	-species <str>		 set the species type, animal or plant or fungi or all; if not set, will compare to all type.
	-seqtype <str>  set the sequence type, pep or cds, default=pep.
	-cuts <int>		cut file with the specified number of seqeunces in each sub file for Interpro,default=100
	-cpu <int>		set th cpu number to use in parallel, default=30
...
if (@Appl == 0)
{
	$appl_parameter = "--appl ProDom --appl PRINTS --appl Pfam --appl SMART --appl PANTHER --appl ProSiteProfiles --appl ProSitePatterns ";
}
else
{
	foreach  (@Appl) 
	{ $appl_parameter .= " --appl $_"; }
}
...
#		print OUT "nohup perl $auto_blast/auto_kegg_blast.pl $fasta $Queue $Pro_code $species $seqtype $Cuts $evalue ./ &\n";
		print OUT "nohup perl $auto_blast/auto_kegg_blast.pl $fasta $Queue $species $seqtype $Cuts $evalue ./ &\n";
		print OUT "cd ..\n";
	}
...
#		print OUT "nohup perl $auto_blast/auto_swissprot_blast.pl $fasta $Queue $Pro_code $seqtype $Cuts $evalue ./ &\n";
		print OUT "nohup perl $auto_blast/auto_swissprot_blast.pl $fasta $Queue $seqtype $Cuts $evalue ./ &\n";
		print OUT "cd ..\n";
	}
	if (defined $Trembl)
	{
...
#		print OUT "nohup perl $auto_blast/auto_trembl_blast.pl $fasta $Queue $Pro_code $seqtype $Cuts $evalue ./ &\n";
		print OUT "nohup perl $auto_blast/auto_trembl_blast.pl $fasta $Queue $seqtype $Cuts $evalue ./ &\n";
		print OUT "cd ..\n";
	}
	if (defined $COG)
	{
...
#		print OUT "nohup perl $auto_blast/auto_COG_blast.pl $fasta $Queue $Pro_code $seqtype $Cuts $evalue ./ &\n";
		print OUT "nohup perl $auto_blast/auto_COG_blast.pl $fasta $Queue $seqtype $Cuts $evalue ./ &\n";
		print OUT "cd ..\n";
	}
	if (defined $NT)
	{
...
#		print OUT "nohup perl $auto_blast/auto_nt_blast.pl $fasta $Queue $Pro_code $species $seqtype $Cuts $evalue ./ &\n";
		print OUT "nohup perl $auto_blast/auto_nt_blast.pl $fasta $Queue $species $seqtype $Cuts $evalue ./ &\n";
		print OUT "cd ..\n";
	}
	if ( $NR && ! $GO )
	{
...
#		print OUT "nohup perl $auto_blast/auto_nr_blast2GO.pl $fasta $Queue $Pro_code $species $seqtype $Cuts $evalue ./ NO &\n"
		print OUT "nohup perl $auto_blast/auto_nr_blast2GO.pl $fasta $Queue $species $seqtype $Cuts $evalue ./ NO &\n"
	}
	if ( $NR && $GO )
	{
		print OUT "##########NR && GO##########\n";
#		print OUT "nohup perl $auto_blast/auto_nr_blast2GO.pl $fasta $Queue $Pro_code $species $seqtype $Cuts $evalue ./ GO &\n"
		print OUT "nohup perl $auto_blast/auto_nr_blast2GO.pl $fasta $Queue  $species $seqtype $Cuts $evalue ./ GO &\n"
	}
close(OUT);
